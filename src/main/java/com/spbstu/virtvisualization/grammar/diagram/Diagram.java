...
public class Diagram {
  ...
  public static class SvgContent {
    ...
    private String getConnectorElement(DiagramToSVG diagramToSVG) {
      ...
      String connectorColor = Utils.convertColorToHtml(diagramToSVG.getConnectorColor());
  ...
  String toSVG(DiagramToSVG diagramToSVG) {
    List<Element> elementList = new ArrayList<Element>();
    if(element instanceof Sequence) {
      List<Element> cursorElementList = new ArrayList<Element>();
      for(Element element: ((Sequence) element).getRRElements()) {
        if(element instanceof Break) {
          if(!cursorElementList.isEmpty()) {
            elementList.add(cursorElementList.size() == 1? cursorElementList.get(0): new Sequence(cursorElementList.toArray(new Element[0])));
    ...
    for (int i = 0; i < elementList.size(); i++) {
      ...
      Element.LayoutInfo layoutInfo = element.getLayoutInfo();
    ...
    for(Element element : elementList) {
      Element.LayoutInfo layoutInfo2 = element.getLayoutInfo();
      ...
      element.toSVG(diagramToSVG, xOffset + 5, yOffset, svgContent);
      svgContent.addLineConnector(xOffset + 5 + width2, y1, xOffset + 5 + width2 + 5, y1);
      yOffset += height2 + 10;
    }
    String connectorElement = svgContent.getConnectorElement(diagramToSVG);
