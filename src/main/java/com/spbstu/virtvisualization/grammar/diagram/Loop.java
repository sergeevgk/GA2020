...
public class Loop extends Element {
   ...
   */
  public Loop(Element element, Element loopElement) {
    this(element, loopElement, 0, null);
  }

  public Loop(Element element, Element loopElement, int minRepetitionCount, Integer maxRepetitionCount) {
  ...
  protected void computeLayoutInfo(DiagramToSVG diagramToSVG) {
    ...
    if(minRepetitionCount > 0 || maxRepetitionCount != null) {
      ...
      Font font = diagramToSVG.getLoopFont();
  ...
  protected void toSVG(DiagramToSVG diagramToSVG, int xOffset, int yOffset, SvgContent svgContent) {
    ...
    if(loopElement != null) {
      ...
      loopElement.toSVG(diagramToSVG, loopOffset, yOffset, svgContent);
    ...
    if(cardinalitiesText != null) {
      String cssClass = svgContent.getDefinedCSSClass(Diagram.CSS_LOOP_CARDINALITIES_TEXT_CLASS);
      if(cssClass == null) {
        Font loopFont = diagramToSVG.getLoopFont();
        String loopTextColor = Utils.convertColorToHtml(diagramToSVG.getLoopTextColor());
        cssClass = svgContent.setCSSClass(Diagram.CSS_LOOP_CARDINALITIES_TEXT_CLASS, "fill:" + loopTextColor + ";" + Utils.convertFontToCss(loopFont));
      }
    ...
    element.toSVG(diagramToSVG, xOffset + 20 + (maxWidth - width1) / 2, yOffset2, svgContent);
