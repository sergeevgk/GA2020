...
class LaTeXEmitter(MAPYVisitor):
    ...
    def split_string(string):
        """
            Функция применяется для разделения строки комментария на две группы. К первой группе относятся те символы
            в строке, которые не являются формульными, то есть просто текст на русском языке. Ко второй группе относятся
            формульные символы, то есть текст на английском языке. Для них нужно отдельное форматирование в языке LaTeX.
        """
        begin = 0
        formula_sub = []
